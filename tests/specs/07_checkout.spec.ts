...
test.describe('Checkout Page Tests', () => {
    ...
    async function verifyError(expectedError: string) {
        const alertMsg = await nativeAlert.runActionAndGetMessage(async () => {
            await checkoutPage.clickBillingContinue({ timeout: 1500 });
        }, 1500);

        if (!alertMsg) {
        throw new Error("Expected validation error, but checkout continued");
        }

        expect(alertMsg).toContain(expectedError);
    }
    ...
    test("Checkout-001: Checkout successfully", async () => {
    ...
    test("Checkout-003: Leave First name empty", async () => {
        ...
        await verifyError(checkoutData.ERROR_MSG.firstNameRequired);
        await checkoutPage.page.waitForTimeout(1500);

    });

    test("Checkout-004: Leave Last name empty", async () => {
        ...
        await verifyError(checkoutData.ERROR_MSG.lastNameRequired);
    });

    test("Checkout-005: Leave Email empty", async () => {
        ...
        await verifyError(checkoutData.ERROR_MSG.emailRequired);
    });
    ...
    test("Checkout-006: Input invalid Email format", async () => {
        ...
        await verifyError(checkoutData.ERROR_MSG.invalidEmail);
    });
    ...
    test("Checkout-007: Do not select a country", async () => {
        ...
        await verifyError(checkoutData.ERROR_MSG.countryRequired);
    });
    ...
    test("Checkout-008: Leave City empty", async () => {
        ...
        await verifyError(checkoutData.ERROR_MSG.cityRequired);
    });
    ...
    test("Checkout-009: Leave Address 1 empty", async () => {
        ...
        await verifyError(checkoutData.ERROR_MSG.streetAddressRequired);
        await checkoutPage.page.waitForTimeout(1500);
    });

    test.only("Checkout-010: Input alphabetic characters in Zip", async () => {
        ...
        await verifyError(checkoutData.ERROR_MSG.zipCodeNumeric);
        await checkoutPage.page.waitForTimeout(1500);
    });

    test.only("Checkout-011: Input special characters in Zip", async () => {
    ...
    test("Checkout-012: Leave Zip/postal code empty", async () => {
        ...
        await verifyError(checkoutData.ERROR_MSG.zipCodeRequired);
        await checkoutPage.page.waitForTimeout(1500);
    });
    test.only(" Checkout-013: Input alphabetic characters in Phone", async () => {
        logTitle("Checkout-013: Input alphabetic characters in Phone");
        ...
        const billing = {
            ...
            address2: "",
            zip: checkoutData.VALID.ZIP_CODE,
            phone: checkoutData.INVALID.PHONE_ALPHA,
        };

        await checkoutPage.fillBillingAddress(billing);

        await verifyError(checkoutData.ERROR_MSG.phoneInvalidFormat);
        await checkoutPage.page.waitForTimeout(1500);
    });

    test.only("Checkout-014: Input special characters in Phone", async () => {
    ...
    test.only("Checkout-015: Input invalid phone length short", async () => {
        ...
        await verifyError(checkoutData.ERROR_MSG.phoneLength);
        await checkoutPage.page.waitForTimeout(1500);
    });

    test.only("Checkout-016: Input invalid phone length long", async () => {
    ...
    test("Checkout-017: Leave Phone empty", async () => {
        ...
        await verifyError(checkoutData.ERROR_MSG.phoneRequired);
        await checkoutPage.page.waitForTimeout(1500);
    });
});